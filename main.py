...
if __name__ == '__main__':
    ...
    if uploaded_file is not None:
        ...
        p=r"C:\\Users\\babao\\Documents\\one drive\\OneDrive\Desktop\\data\\data"    
        if os.path.isdir(p):
            for file in os.listdir(p):
                file_name, file_ext = os.path.splitext(file)
                if file_name.lower() == name.lower() and file_ext == '.txt':
                    matched_file = os.path.join(p, file)
                    break
        ...
        if matched_file:
            ...
            result = f'<p style="font-family:sans-serif; color:White; font-size: 16px;">' \
                    f'The given image is {name}.<br>Details:<br>{file_content}</p>'
        else:
            result = f'<p style="font-family:sans-serif; color:White; font-size: 16px;">' \
                    f'The given image is {name}, but no matching file was found in the directory.</p>'
